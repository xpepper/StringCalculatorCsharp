...
namespace StringCalculatorKata.Tests
{
    public class StringCalculatorTest
    {
        ...
        {
            var exception = Assert.Throws<Exception>(() => new StringCalculator(new DummyLogger(), null).Add(("-1,1,-4")));
        ...
        public void Logging_add_results()
        {
            ...
            StringCalculator sc = new StringCalculator(logger.Object, null);
        ...
        [Fact]
        public void Logger_Has_Crashed_Report_Error()
        {
            var webServiceCaller = new Mock<IWebService>();

            var logger = new Mock<ILogger>();
            logger.Setup(x => x.Write(It.IsAny<string>())).Throws(new Exception("Something went wrong!"));

            StringCalculator sc = new StringCalculator(logger.Object, webServiceCaller.Object);

            sc.Add("3,1");

            webServiceCaller.Verify(x => x.ReportError("Something went wrong!", "Writing log failed with error."));
        }

        private static void CheckAdd(string stringOfNumbers, int expectedSum)
        {
            ILogger dummyLogger = new DummyLogger();
            new StringCalculator(dummyLogger, null).Add(stringOfNumbers).Should().Be(expectedSum);
        }
    }
